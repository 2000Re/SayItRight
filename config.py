...
PICK_N = 1           # 今回の動画用に実際に選ぶ件数(1回の実行でアップロードする本数)
MIN_LETTERS = 4      # これより短い単語は除外
# 直近何件の投稿と綴りパターンの重複を避けるか。上位スコアの候補が
# ough/eigh/kn- など一部のパターンに偏りやすく、ウィンドウが広すぎると
# 直近runでそれらを使い切ってしまい、プールが1〜2件まで枯渇して
# PICK_N分の候補が選べなくなることがあった(TRICKY_PATTERNSにパターンを
# 追加して偏りは緩和したが、念のためウィンドウ自体も短縮しておく)。
# 1日9回の実行(PICK_N=1)を想定し、直近1日分(9回×1件=9件)をカバーする
# 長さにしている(1日3回・PICK_N=3だった頃も1日3回×3件=9件で同じ意味
# だったため、1日あたりの総投稿数が変わらない限りこの値のままでよい)。
RECENT_PATTERN_WINDOW = 9
